		...
		switch ( penMsgType )
		{
			...
			case PenMsgType.PEN_SETUP_AUTO_SHUTDOWN_RESULT:
			{
				try
				{
					...
					Log.d( TAG, "pen setup auto shutdown result : " + job.getBoolean( Const.JsonTag.BOOL_RESULT ) );
				}
			...
			case PenMsgType.PEN_SETUP_SENSITIVITY_RESULT:
			{
				...
				{
					JSONObject job = new JSONObject( content );
					Log.d( TAG, "pen setup sensitivity result : " + job.getBoolean( Const.JsonTag.BOOL_RESULT ) );
				}
				catch ( JSONException e )
				{
					e.printStackTrace();
			...
			case PenMsgType.PEN_SETUP_AUTO_POWER_ON_RESULT:
			{
				...
				{
					JSONObject job = new JSONObject( content );
					Log.d( TAG, "pen auto power on setting result : " + job.getBoolean( Const.JsonTag.BOOL_RESULT ) );
				}
				catch ( JSONException e )
				{
					e.printStackTrace();
			...
			case PenMsgType.PEN_SETUP_BEEP_RESULT:
			{
				...
				{
					JSONObject job = new JSONObject( content );
					Log.d( TAG, "beep on/off setting result : " + job.getBoolean( Const.JsonTag.BOOL_RESULT ) );
				}
				catch ( JSONException e )
				{
					e.printStackTrace();
			...
			case PenMsgType.PEN_SETUP_PEN_COLOR_RESULT:
			{
				...
				{
					JSONObject job = new JSONObject( content );
					Log.d( TAG, "pen color setting result : " + job.getBoolean( Const.JsonTag.BOOL_RESULT ) );
				}
				catch ( JSONException e )
				{
					e.printStackTrace();
